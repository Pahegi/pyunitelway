"""Tests for the UNI-TE answer parsers.

Parsers receive the de-duplicated UNI-TE answer bytes produced by
``conversion.unwrap_unite_response`` (answer code first). Layouts follow
NUM 1060 "Use of the UNI-TE protocol" 938914; values are little-endian
(low byte first, 938914 section 2).
"""
...
class TestParseAvailableBytesInRam:
    # 938914 section 4.15: H'F5' / H'77' / status / 1 long word

    def test_count_with_dle_byte_inside(self):
        # 0x000110D0 = 69840 — the 0x10 byte inside the long word must survive
        # (the old double de-duplication pass ate the byte after it)
...
def test_parse_mirror_result():
    # 938914: the mirror answer echoes the request data
...
def test_parse_write_result():
    # positive report for write requests is H'FE'
    assert parse_write_result([0xFE]) is True
    assert parse_write_result([0x66]) is False


class TestParseLadderReadResponse:
    # Pins the current implementation contract: answer bytes = answer code,
    # object type, then the object bytes. The bit path extracts bit n of the
    # value byte; sizes arrive as the characters '0'..'7'/'B'/'W'/'L'/'&'.
...
def test_parse_unit_identification():
    # 938914: product type / subtype / version / text
...
def test_parse_unit_fault_history():
    # four 16-bit counters, low byte first
